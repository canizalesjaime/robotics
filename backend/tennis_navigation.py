...
def draw_ball_contour(binary_image, contours, img_pub):
    ...
    for c in contours:
        ...
        if (area>100):
            cv2.drawContours(cv_rgb, [c], -1, (150,250,150), 1)
            cv2.drawContours(black_image, [c], -1, (150,250,150), 1)
            cx, cy = get_contour_center(c)
            cv2.circle(cv_rgb, (cx,cy),(int)(radius),(0,0,255),1)
    ...
    #cv2.waitKey(1)
    ros_tennis_img = bridge.cv2_to_imgmsg(cv_rgb, "bgr8")
...
def main(node):
    # Subscribers:
    depth_sub = message_filters.Subscriber(node, Image, "/head_camera/depth/image")
    rgb_sub = message_filters.Subscriber(node, Image, "/head_camera/rgb/image_raw")
    ...
    img_pub = node.create_publisher(Image, "/tennis_circle_img", 10)
    pose_pub = node.create_publisher(PoseArray, '/tennis_pose', 10)    
    ...
    while rclpy.ok():
        ...
        if cv_rgb is not None:
            global update_imgs
            update_imgs = False
            hsv_mask = filter_color()
            contours = getContours(hsv_mask)
            draw_ball_contour(hsv_mask, contours, img_pub)
